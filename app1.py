"""
app1.py - Alternative version of the MineStar Reporting app
This version uses a constant 24 hours for total time and calculates operating hours
by subtracting all downtime categories (DELAY, EXTENDED LOSS, PLANNED DOWN, STANDBY, UNPLANNED DOWN)
from the total 24 hours.

Formula changes:
- Grand Total is constant at 24.0 hours per unit
- Operating hrs = 24.0 - (DELAY + EXTENDED LOSS + PLANNED DOWN + STANDBY + UNPLANNED DOWN)
- PA and UA formulas remain the same
"""
...
def create_performance_sheet(delay_df, cycle_df):
    ...
    with constant 24 hours total time and operating hours calculated by subtraction
    """
    if delay_df is None:
        return None
    ...
    units = set()
    if cycle_df is not None and 'Unit' in cycle_df.columns:
    ...
    # Set constant Grand Total for each unit (24 hours)
    ...
    # Calculate Operating hrs by subtracting all delay categories from the constant 24 hours
    performance_df['Operating hrs'] = (performance_df['Grand Total'] - 
    ...
    # Ensure Operating hrs doesn't go below 0
    performance_df['Operating hrs'] = performance_df['Operating hrs'].apply(lambda x: max(0, round(x, 2)))
    
    # Calculate PA using formula: PA = (Operation hrs+DELAY+STANDBY) / (Grand Total-EXTENDED LOSS)*100
    ...
    grand_total_row = pd.DataFrame({
        ...
        'Grand Total': [24.0 * len(units)],  # Total hours for all units (24 * number of units)
...
def process_excel_file(file):
    ...
    try:
        ...
        # Create Performance sheet if we have Delay data
        # Note: In app1.py, cycle data is optional since we use constant 24 hours model
        if delay_df is not None:
            log_message("Membuat sheet Performance berdasarkan data Delay")
